...
APP_INSIGHTS_CONN=$(az keyvault secret show --vault-name "$KEY_VAULT" --name "appinsights-auth-service" --query "value" -o tsv 2>/dev/null || echo "")
[ -n "$APP_INSIGHTS_CONN" ] && print_success "  appinsights-auth-service: retrieved" || print_warning "  appinsights-auth-service: not configured (telemetry disabled)"
